...
from app.application.dto.cart_dto import AddItemCommand, UpdateItemCommand
...
class FakeCartRepo:
    ...
    def _make(self, user_id: uuid.UUID) -> CartEntity:
        ...
        cart = CartEntity(id=uuid.uuid4(), user_id=user_id, status=CartStatus.ACTIVE, created_at=now, updated_at=now)
...
class FakeCartItemRepo:
    ...
    async def get_by_cart_and_product(self, cart_id: uuid.UUID, product_id: uuid.UUID) -> CartItemEntity | None:
        return next((i for i in self._items if i.cart_id == cart_id and i.product_id == product_id), None)

    async def create(self, data: CreateCartItem) -> CartItemEntity:
        now = datetime.datetime.now(datetime.UTC)
        item = CartItemEntity(id=uuid.uuid4(), cart_id=data.cart_id, product_id=data.product_id, quantity=data.quantity, unit_price=data.unit_price, created_at=now, updated_at=now)
    ...
    async def update_quantity(self, item_id: uuid.UUID, quantity: int) -> CartItemEntity:
        for i, item in enumerate(self._items):
            if item.id == item_id:
                updated = CartItemEntity(id=item.id, cart_id=item.cart_id, product_id=item.product_id, quantity=quantity, unit_price=item.unit_price, created_at=item.created_at, updated_at=datetime.datetime.now(datetime.UTC))
...
def _make_product(product_id: uuid.UUID, status: str = "active", price: str = "50000") -> ProductInfo:
    return ProductInfo(id=product_id, name="Test Product", price=Decimal(price), status=status, image_url=None)
...
async def test_add_item_creates_cart_and_item():
    ...
    user_id = uuid.uuid4()
    service = CartService(FakeCartRepo(), FakeCartItemRepo(), FakeProductClient({product_id: _make_product(product_id)}))
...
async def test_add_item_product_not_active():
    product_id = uuid.uuid4()
    service = CartService(FakeCartRepo(), FakeCartItemRepo(), FakeProductClient({product_id: _make_product(product_id, status="draft")}))
